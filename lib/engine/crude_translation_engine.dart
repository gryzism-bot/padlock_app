...
class CrudeTranslationEngine {
  ...
  String translateSentence({
    ...
    final actionTranslation = state.action.translations[language];
    for (final form in {
      state.action.infinitive,
      state.action.presentThirdPerson,
      state.action.pastSimple,
      state.action.pastParticiple,
      state.action.ingForm,
    }) {
    ...
    for (final entry in _grammarWordTranslations(language).entries) {
...
Map<String, String> _grammarWordTranslations(Language language) {
  if (language != Language.pl) {
    return const {};
  }
