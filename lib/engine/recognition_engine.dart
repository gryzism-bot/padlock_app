...
class RecognitionEngine {
  ...
  void _trimParticipantBoundaries(_RecognitionBuilder builder) {
    ...
    if (_crossesPhrase(
      ...
      builder.sourceEnd = firstPhraseStart - 1;
    }
  ...
  void _buildParticipants(_RecognitionBuilder builder) {
    ...
    if (builder.destinationStart >= 0 &&
        builder.destinationEnd >= builder.destinationStart) {
      builder.destination = _recognizeNounPhrase(
        builder.tokens.sublist(
          builder.destinationStart,
  ...
  void _recognizeDestinationPhrase(
    _RecognitionBuilder builder,
    List<String> tokens,
  ) {
    ...
    final toIndex = builder.verbChainEnd + 1 + wordsBefore;
    final destinationStart = toIndex + 1;

    if (!_looksLikeCompanionPhrase(builder, destinationStart)) {
      return;
    }

    builder.destinationStart = destinationStart;
  ...
  void _recognizeSourcePhrase(
    ...
    final wordsBefore = _phraseWordIndex(tokens, 'from');

    if (wordsBefore < 0) {
      return;
    }

    final fromIndex = builder.verbChainEnd + 1 + wordsBefore;
    final sourceStart = fromIndex + 1;

    if (!_looksLikeCompanionPhrase(builder, sourceStart)) {
      return;
    }

    builder.sourceStart = sourceStart;
    builder.sourceEnd = _nounPhraseEnd(builder, sourceStart);
  }
  ...
  int _toWordIndexNotAlreadyReserved(
    ...
    for (var index = 0; index < tokens.length; index++) {
      if (tokens[index].toLowerCase() != 'to') {
        continue;
      }
