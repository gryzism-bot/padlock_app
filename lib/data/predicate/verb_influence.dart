...
List<PredicateInfluence> predicateInfluencesFor(Verb action) {
  ...
  if (action.takesBeneficiary) {
    add(_predicateProperty(action, 'beneficiary', 'beneficiary', 43));
  }

  if (action.takesSource) {
    add(_predicateProperty(action, 'source', 'source', 42));
  }
...
PredicateInfluence _influenceForPath(
  ...
  return switch (kind) {
    ...
    PredicatePathKind.fromSource => _predicateProperty(
      action,
      'source',
      'source',
      42,
    ...
    PredicatePathKind.onLocation => _predicateProperty(
      action,
      'on-location',
      'on location',
      35,
